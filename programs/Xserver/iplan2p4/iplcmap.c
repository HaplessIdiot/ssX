...
typedef struct _iplVisuals {
    struct _iplVisuals	*next;
    int			depth;
    int			bitsPerRGB;
    int			visuals;
    int			count;
} iplVisualsRec, *iplVisualsPtr;

static int  iplVisualPriority[] = {
    PseudoColor, DirectColor, GrayScale, StaticColor, TrueColor, StaticGray
};

#define NUM_PRIORITY	6

static iplVisualsPtr	iplVisuals;
...
iplSetVisualTypes (depth, visuals, bitsPerRGB)
    ...
    int	    visuals;
{
    iplVisualsPtr   new, *prev, v;
    int		    count;

    new = (iplVisualsPtr) xalloc (sizeof *new);
    if (!new)
	return FALSE;
    new->next = 0;
    new->depth = depth;
    ...
    new->count = count;
    for (prev = &iplVisuals; v = *prev; prev = &v->next);
    *prev = new;
    return TRUE;
}
    ...
    int		bitsPerRGB;
{
    ...
    iplVisualsPtr   visuals, nextVisuals;
    ...
    if (!iplVisuals) 
    {
		...
		vtype = 0;
	    if (!iplSetVisualTypes (d, vtype, bitsPerRGB))
		return FALSE;
    	}
    }
    nvisual = 0;
    ndepth = 0;
    for (visuals = iplVisuals; visuals; visuals = nextVisuals) 
    {
    ...
    *nvisualp = nvisual;
	...
	for (i = 0; i < NUM_PRIORITY; i++) {
	    if (! (vtype & (1 << iplVisualPriority[i])))
		continue;
	    visual->class = iplVisualPriority[i];
