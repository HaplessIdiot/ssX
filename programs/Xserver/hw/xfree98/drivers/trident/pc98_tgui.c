...
/* $XFree86: xc/programs/Xserver/hw/xfree98/vga256/drivers/trident/pc98_tgui.c,v 3.7 1997/01/25 04:21:26 dawes Exp $ */
...
Bool ChipInit(void)
{
  ...
  tmp = 0;
  while(100 > tmp && (inb(GER_STATUS) & 0x20) == 0x20) tmp++;

  tmp = 0;
  while(100 > tmp && (inb(GER_STATUS) & GE_BUSY) == GE_BUSY){
    tmp++;
    ErrorF("GE waiting Idel %d %02X\n",tmp,inb(GER_STATUS));
  }
  ...
  tmp = 0;
  while(100 > tmp && (inb(GER_STATUS) & GE_BUSY) == GE_BUSY) tmp++;
  if(tmp==100) FatalError("GE Freeze");
