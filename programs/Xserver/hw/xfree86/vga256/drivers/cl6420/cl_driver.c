 ...
 *
 * Cirrus Logic GD-6420 Notebook Graphics Controller extensions,
...
typedef struct {
	...
	unsigned char DM, CCLK, WRC, CLK;	/* CRTC Extensions */
...
} vgaCL6420Rec, *vgaCL6420Ptr;

/*
 * Forward definitions for the functions that make up the driver.  See
 * the definitions of these functions for the real scoop.
 */
static Bool     CL6420Probe();
static char *   CL6420Ident();
static Bool     CL6420ClockSelect();
static void     CL6420EnterLeave();
static Bool     CL6420Init();
static void *   CL6420Save();
static void     CL6420Restore();
static void     CL6420Adjust();
/*
 * These are the bank select functions.  There are defined in bank.s
 */
extern	void     CL6420SetRead();
extern	void     CL6420SetWrite();
extern	void     CL6420SetReadWrite();

#define	CL6420_LOCK_REG		0x0A
 ...
 */
#define new ((vgaCL6420Ptr)vgaNewVideoState)
 ...
 *
 * The CLK register of the CL6420 can be programmed to overrule the
 ...
 */
#define	CL6420_MAX_CLOCK_IN_KHZ	45000
#define	CL6420_N_CLOCKS		16
#define	CL6420_LCD_CLOCK	1
#define	CL6420_SIM_CLOCK	4
...
int no;
{
	...
	default:
		 ...
		 * If SimulSCAN (LCD + CRT) is selected allow only 25.172 MHz
		 */
		outb(0x3CE, 0x81);
		mode = inb(0x3CF);
		if (mode & 0x80) {
			if (no != CL6420_SIM_CLOCK)
				return (FALSE);
		}
		else if (mode & 0x01) {
			if (no != CL6420_LCD_CLOCK)
				return (FALSE);
		}
#endif
		outb(0x3CE, 0x84);
...
CL6420Probe()
{
	...
	if (vga256InfoRec.chipset)
	{
		 ...
		 */
		if (StrCaseCmp(vga256InfoRec.chipset, CL6420Ident(0)))
			return (FALSE);
      		else
			CL6420EnterLeave(ENTER);
    	}
  	else
	{
		/*
		 ...
		 */
		outb(0x3CE, CL6420_LOCK_REG);
		if (inb(0x3CF) != 0x01)
		{
			CL6420EnterLeave(LEAVE);
			return(FALSE);
		}
		 ...
		 * The first nibble is the CHIP SET ID ??
		 *	 2:	CL-GD510/520
		 *	 3:	CL-GD610/620
		 *	 4:	CL-GD5320
		 *	 5:	CL-GD6410
		 *	 6:	CL-GD5410
		 *	 7:	CL-GD6420
		 * The second nibble is the revision
		 */
		outb(0x3CE, 0xAA); design = inb(0x3CF);
		outb(0x3CE, 0xAB); mask = inb(0x3CF);

		if ((design & 0xF0) != (mask & 0xF0)  ||
				((design & 0xF0) >> 4) != 7) 
		{
	  		CL6420EnterLeave(LEAVE);
	  		return(FALSE);
	 ...
	 */
  	if (!vga256InfoRec.clocks)
		 ...
		 */
      		vgaGetClocks(CL6420_N_CLOCKS, CL6420ClockSelect);
    	}
	 ...
	 * setting.
	 */
	vga256InfoRec.maxClock = CL6420_MAX_CLOCK_IN_KHZ;
	 ...
	 */
  	vga256InfoRec.chipset = CL6420Ident(0);
  	vga256InfoRec.bankedMono = TRUE;

	outb(0x3CE, 0x81);
	switch (inb(0x3CF) & 0x81) {	/* Display Mode */
	case 0x80:
	case 0x81:
		ErrorF("SimulSCAN operation (LCD + CRT)\n");
		break;
	case 0x01:
		ErrorF("LCD panel display only\n");
		break;
	case 0x00:
		ErrorF("CRT display only\n");
		break;
	}
  	return(TRUE);
}
 ...
 * the registers that have previously been saved in the vgaCL6420Rec data 
 * structure.
 *
 ...
 */
static void 
CL6420Restore(restore)
vgaCL6420Ptr restore;
{
	unsigned char	temp;
	...
	outb(0x3CE, 0x81);
	temp = inb(0x3CF) & 0xFB;
	outb(0x3CF, restore->DM | temp);/* Display Mode (interlaced bit) */
 ...
 * into the vgaCL6420Rec data structure.  There is in general no need to
...
vgaCL6420Ptr save;
{
	 ...
	 */
	save = (vgaCL6420Ptr)vgaHWSave((vgaHWPtr)save, sizeof(vgaCL6420Rec));

	/*
	 * The port I/O code necessary to read in the extended registers 
	 * into the fields of the vgaCL6420Rec structure goes here.
	 */
	outb(0x3CE, 0x62);
	...
	outb(0x3CE, 0x84);
	save->CLK = inb(0x3CF);		/* Clock Select Register */
 ...
 * function fills in the vgaCL6420Rec with all of the register values needed
...
DisplayModePtr mode;
{
	 ...
	 */
	if (!vgaHWInit(mode,sizeof(vgaCL6420Rec)))
		return(FALSE);
	...
	outb(0x3CE, 0x81);
	lcd = (inb(0x3CF) & 0x81);	/* Check whether LCD display is on */
	...
	if (new->std.NoClock >= 0) {
		if (new->std.NoClock >= CL6420_N_CLOCKS) {
			ErrorF("Invalid clock, there ain't that many");
			return (FALSE);
		}
		...
		if (lcd & 0x80) {
			if (no != CL6420_SIM_CLOCK) {
				ErrorF("Clock not allowed for LCD display");
				return (FALSE);
			}
		}
		else if (lcd & 0x01) {
...
int x, y;
{
	...
	/* CL6420 specific bits 18,19,20  ??? */
	outb(0x3CE, 0x7C);
	tmp = inb(0x3CF) & 0xF0;
	outb(0x3CF, tmp | ((msb >> 16) & 0x0F));
