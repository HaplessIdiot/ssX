     ...
     DisplayModePtr mode;
{
  ...
  if (xf86bpp >= 8) {
    ...
    new->std.CRTC[19] = vga256InfoRec.displayWidth >> 3; /*we're in byte-mode*/
...
if (xf86bpp >= 8) {
   ...
   * and if the resolution will exceed the bandwidth capability of these 
   * systems.
   */
