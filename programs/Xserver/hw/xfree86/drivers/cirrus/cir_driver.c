  ...
  int clockno;
{
  ...
  if ((vgaBitsPerPixel >=8 && clockval > cirrusClockLimit8bpp[chip]) ||
      (vgaBitsPerPixel < 8 && clockval > cirrusClockLimit4bpp[chip]))
  {
    ErrorF("CIRRUS: clock %7.3f is too high for %s (max is %7.3f)\n",
	   clockval / 1000.0, xf86TokenToString(chipsets, chip),
	   vgaBitsPerPixel >= 8 ? cirrusClockLimit8bpp[chip] : 
				  cirrusClockLimit4bpp[chip] / 1000.0);
     ...
     int chip;
{
     ...
     for (rec = cirrusClockTab; rec < end; rec++)
          if (((CLOCKVAL(rec->numer, rec->denom) > cirrusClockLimit8bpp[chip])
		&& vgaBitsPerPixel >= 8) ||
	      ((CLOCKVAL(rec->numer, rec->denom) > cirrusClockLimit4bpp[chip])
		&& vgaBitsPerPixel < 8))
     ...
     DisplayModePtr mode;
{
  ...
  if (new->std.NoClock >= 0)
	...
	if (vgaBitsPerPixel >= 8 && vgaBitsPerPixel == 24 && 
	(cirrusChip <= CLGD5430 ||
	HAVE75XX()))
	  /* 542x, 5434, 5430, 754x */
