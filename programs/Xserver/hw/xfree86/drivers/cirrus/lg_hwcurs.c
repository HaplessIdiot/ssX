...
static void LgSetCursorColors(ScrnInfoPtr pScrn, int bg, int fg)
{
	...
	volatile CARD8 *pPaletteState = (CARD8 *)(pLg->CirRec.IOBase + PALETTE_STATE);
	volatile CARD8 *pPaletteAddr = (CARD8 *)(pLg->CirRec.IOBase + PALETTE_WRITE_ADDR);
	volatile CARD8 *pPaletteData = (CARD8 *)(pLg->CirRec.IOBase + PALETTE_DATA);
...
{
	LgPtr pLg = LGPTR(pScrn);
	volatile CARD16 *pXPos = (CARD16 *)(pLg->CirRec.IOBase + CURSOR_X_POS);
	volatile CARD16 *pYPos = (CARD16 *)(pLg->CirRec.IOBase + CURSOR_Y_POS);
	volatile CARD16 *pPreset = (CARD16 *)(pLg->CirRec.IOBase + CURSOR_PRESET);
	...
	if (x < 0 || y < 0) {
		...
		*pPreset = newPreset;
		pLg->CirRec.CursorIsSkewed = TRUE;
	} else if (pLg->CirRec.CursorIsSkewed) {
		/* Reset the hotspot location. */
		*pPreset &= 0x8080;
		pLg->CirRec.CursorIsSkewed = FALSE;
	}
...
static void LgLoadCursorImage(ScrnInfoPtr pScrn, unsigned char *bits)
{
	...
	volatile CARD32 *pHOSTDATA = (CARD32 *)(pLg->CirRec.IOBase + HOSTDATA);
	volatile CARD8 *pQFREE = (CARD8 *)(pLg->CirRec.IOBase + QFREE);
	int l, w;
...
{
	LgPtr pLg = LGPTR(pScrn);
	volatile CARD16 *pCursorControl = (CARD16 *)(pLg->CirRec.IOBase + CURSOR_CONTROL);
...
void LgShowCursor(ScrnInfoPtr pScrn)
{
	LgPtr pLg = LGPTR(pScrn);
	volatile CARD16 *pCursorControl = (CARD16 *)(pLg->CirRec.IOBase + CURSOR_CONTROL);
	volatile CARD16 *pCursorAddr = (CARD16 *)(pLg->CirRec.IOBase + CURSOR_ADDR);
...
Bool LgHWCursorInit(ScreenPtr pScreen)
{
	...
	pLg->CirRec.CursorInfoRec = infoPtr;
	LgFindCursorTile(pScrn, &pLg->HWCursorImageX, &pLg->HWCursorImageY,
