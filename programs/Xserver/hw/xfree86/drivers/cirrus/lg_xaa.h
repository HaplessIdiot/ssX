...
enum {						/* OR these together to form a bitBLT mode */
	...
	PATeqSRC		= 0x0800,	/* The Pattern and Source operands are the
								   same */
...
#define LgREADY() \
	((*(volatile CARD8 *)(pLg->CirRec.IOBase + STATUS) & 0x07) == 0x00)

#define LgSETROP(rop) \
	*(CARD16 *)(pLg->CirRec.IOBase + DRAWDEF) = (rop);

#define LgSETTRANSPARENCY(trans) \
	*(CARD16 *)(pLg->CirRec.IOBase + DRAWDEF) = \
	(trans) | (*(volatile CARD16 *)(pLg->CirRec.IOBase + DRAWDEF) & 0x00FF);

#define LgSETMODE(mode) \
	*(CARD16 *)(pLg->CirRec.IOBase + BLTDEF) = (mode);

#define LgSETDSTXY(X, Y) \
	*(CARD32 *)(pLg->CirRec.IOBase + OP0_opRDRAM) = (((Y) << 16) | (X));

#define LgSETSRCXY(X, Y) \
	*(CARD32 *)(pLg->CirRec.IOBase + OP1_opRDRAM) = (((Y) << 16) | (X));

#define LgSETPATXY(X, Y) \
	*(CARD32 *)(pLg->CirRec.IOBase + OP2_opRDRAM) = (((Y) << 16) | (X));

#define LgSETTRANSMASK(X, Y) LgSETPATXY(X, Y)

#define LgSETSRAMDST(offset) \
	*(CARD16 *)(pLg->CirRec.IOBase + OP0_opSRAM) = (offset);

#define LgSETSRAM1OFFSET(offset) \
	*(CARD16 *)(pLg->CirRec.IOBase + OP2_opSRAM) = (offset);

#define LgSETSRAM2OFFSET(offset) \
	*(CARD16 *)(pLg->CirRec.IOBase + OP2_opSRAM) = (offset);

#define LgSETMSRAM1OFFSET(offset) \
	*(CARD16 *)(pLg->CirRec.IOBase + OP1_opMSRAM) = (offset);

#define LgSETMSRAM2OFFSET(offset) \
	*(CARD16 *)(pLg->CirRec.IOBase + OP2_opMSRAM) = (offset);

#define LgSETMDSTXY(X, Y) \
	*(CARD32 *)(pLg->CirRec.IOBase + OP0_opMRDRAM) = (((Y) << 16) | (X));

#define LgSETMSRCXY(X, Y) \
	*(CARD32 *)(pLg->CirRec.IOBase + OP1_opMRDRAM) = (((Y) << 16) | (X));

#define LgSETMPATXY(X, Y) \
	*(CARD32 *)(pLg->CirRec.IOBase + OP2_opMRDRAM) = (((Y) << 16) | (X));

#define LgSETMTRANSMASK(X, Y) LgSETMPATXY(X, Y)

#define LgSETPHASE0(phase) \
	*(CARD32 *)(pLg->CirRec.IOBase + OP0_opRDRAM) = (phase);

#define LgSETPHASE1(phase) \
	*(CARD32 *)(pLg->CirRec.IOBase + OP1_opRDRAM) = (phase);

#define LgSETPHASE2(phase) \
	*(CARD32 *)(pLg->CirRec.IOBase + OP2_opRDRAM) = (phase);

#define LgSETMPHASE0(phase) \
	*(CARD32 *)(pLg->CirRec.IOBase + OP0_opMRDRAM) = (phase);

#define LgSETMPHASE1(phase) \
	*(CARD32 *)(pLg->CirRec.IOBase + OP1_opMRDRAM) = (phase);

#define LgSETEXTENTS(width, height)  \
	*(CARD32 *)(pLg->CirRec.IOBase + BLTEXT_EX) = (((height) << 16)|(width));

#if 0
#define LgSETMEXTENTS(width, height)  \
	*(CARD32 *)(pLg->CirRec.IOBase + MBLTEXT_EX) = (((height) << 16)|(width));
#else
...
#define LgSETMEXTENTS(width, height)  \
	{ \
		*(CARD16 *)(pLg->CirRec.IOBase + MONOQW) = ((width + 7) >> 3);  \
		*(CARD32 *)(pLg->CirRec.IOBase + MBLTEXT_EX) = \
			(((height) << 16)|(width));  \
	}
...
/*
	*(CARD16 *)(pLg->CirRec.IOBase + MBLTEXT_EX) = height;
	*(CARD16 *)(pLg->CirRec.IOBase + MBLTEXT_EX + 2) = width;
 */
#endif

#define LgHOSTDATAWRITE(data)  \
	*(CARD32 *)(pLg->CirRec.IOBase + HOSTDATA) = (data);

#define LgHOSTDATAREAD()  \
	(*(CARD32 *)(pLg->CirRec.IOBase + HOSTDATA))

#define LgSETBACKGROUND(color) \
	*(CARD32 *)(pLg->CirRec.IOBase + OP_opBGCOLOR) = (color);

#define LgSETFOREGROUND(color) \
	*(CARD32 *)(pLg->CirRec.IOBase + OP_opFGCOLOR) = (color);

#define LgSETPATOFF(xoff, yoff) \
	*(CARD16 *)(pLg->CirRec.IOBase + PATOFF) = (((yoff) << 8) | (xoff));

#define LgSETSWIZZLE() \
	*(CARD16 *)(pLg->CirRec.IOBase + bltCONTROL) |= 0x0400;

#define LgCLEARSWIZZLE() \
	*(CARD16 *)(pLg->CirRec.IOBase + bltCONTROL) &= ~0x0400;

#define LgSETBITMASK(m) \
	*(unsigned int *)(pLg->CirRec.IOBase + BITMASK) = m;
