/* $XFree86: xc/programs/Xserver/hw/xfree86/drivers/tseng/tseng_cursor.c,v 1.7.2.1 1998/07/24 11:36:34 dawes Exp $ */
    ...
    CursorPtr pCurs;
{
    ...
    *pPriv = (pointer) ram;
    ...
    h = pCurs->bits->height;
    if (h > tsengCursorHeight)
	h = tsengCursorHeight;
    w = pCurs->bits->width;
    if (w > tsengCursorWidth)
	w = tsengCursorWidth;
...
static Bool 
TsengUnrealizeCursor(pScr, pCurs)
    ScreenPtr pScr;
    CursorPtr pCurs;
{
    pointer priv;
...
static void 
TsengSetCursor(pScr, pCurs, x, y, generateEvent)
    ScreenPtr pScr;
    CursorPtr pCurs;
    int x, y;
    Bool generateEvent;
...
get_et6000_color_bits(unsigned short col_in, int *badColour)
{
...
static void 
TsengRecolorCursor(pScr, pCurs, displayed)
    ScreenPtr pScr;
    CursorPtr pCurs;
    Bool displayed;
{
...
void 
TsengQueryBestSize(class, pwidth, pheight, pScreen)
    int class;
