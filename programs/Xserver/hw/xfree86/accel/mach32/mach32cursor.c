...
static mach32CursGeneration = -1;
static mach32SaveCursors[MAXSCREENS];
     ...
     CursorPtr pCurs;
{
  ...
  ram = (unsigned short *)Xcalloc(MACH32_CURSBYTES);
  *pPriv = (pointer)ram;
  if (!ram) 
     return (FALSE);
  ...
  memset(ram, 0xaa, MACH32_CURSBYTES);
  ...
  for(i=0; i < bits->height; i++) {
	...
	case 1:
		*ram++ = cursor_mask[*pServMsk & 0xff] |
			 cursor_lookup[*pServSrc & 0xff];
		*ram++ = cursor_mask[*pServMsk++ >> 8] |
     ...
     Bool      generateEvent;
{
  ...
  MemToBus(mach32CursorMemory, pCurs->bits->devPriv[index], 1024);
     ...
     int       x, y;
{
	...
	outw(HORZ_CURSOR_OFFSET, xoff);
	outw(VERT_CURSOR_OFFSET, yoff);
