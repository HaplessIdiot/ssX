    ...
    int                 planemask;
{
    ...
    if ( (alu != MIX_SRC) 
       ...
       agxMemToVid( (x + j * agxVirtX) << BytesPerPixelShift, 
                    w << BytesPerPixelShift,
                    psrc, 
                    pwidth,
                    h );
    }
    ...
    unsigned int 	planemask;
{
    ...
    srcBWidthShift = agxBytePadScratchMapPow2( pw, BytesPerPixelShift );
    srcBWidth = 1 << srcBWidthShift;
    srcPWidth = srcBWidth >> BytesPerPixelShift;
    srcMaxLines = agxScratchSize >> srcBWidthShift;
...
void
agxImageStipple(x, y, w, h, psrc, pwidth, pw, ph, pox, poy, fgPixel, alu, planemask)
    int			x;
    int			y;
    int			w;
    int			h;
...
#ifndef NO_MULTI_IO
    GE_OUT_W(GE_FRGD_MIX, (MIX_DST << 8) | alu );  /* both fg & bg */
#else
    GE_OUT_B(GE_FRGD_MIX, alu );
...
#ifndef NO_MULTI_IO
    GE_OUT_W(GE_FRGD_MIX, (alu << 8) | alu );  /* both fg & bg */
#else
    GE_OUT_B(GE_FRGD_MIX, alu );
