...
Modified for the DEC TGA by Alan Hourihane (alanh@fairlite.demon.co.uk)
...
miBSFuncRec tgaBSFuncRec;

miBSFuncRec tgaBSFuncRec8 = {
    cfbSaveAreas,
    cfbRestoreAreas,
    (void (*)()) 0,
    (PixmapPtr (*)()) 0,
...
Bool
tgaScreenInit(pScreen, pbits, xsize, ysize, dpix, dpiy, width)
    ...
    int width;			/* pixel width of frame buffer */
{
  ...
  switch (tgaInfoRec.bitsPerPixel) 
	...
	if (tgaInfoRec.bitsPerPixel > 8) {
	    if (!cfbSetVisualTypes(tgaInfoRec.depth, 1 << TrueColor,
				   bitsPerRGB))
		return FALSE;
	}
	if (!cfbInitVisuals(&visuals, &depths, &nvisuals, &ndepths, &rootdepth,
			    &defaultVisual,
			    1<<(tgaInfoRec.bitsPerPixel - 1), bitsPerRGB))
	  return FALSE;
  ...
  switch (rootdepth)
	...
	return FALSE;
       ...
       * StoreColors in tga.c */
      mfbRegisterCopyPlaneProc (pScreen, cfbCopyPlane);
      break;
    case 15:
    case 16:
      tgaBSFuncRec = tgaBSFuncRec16;
      ...
      if (!tgaNoAccel)
	{
	  /***** ACCELERATED *****/
	  pScreen->CopyWindow = tgaCopyWindow;
	  pScreen->CreateGC = tgaCreateGC16;
          pScreen->PaintWindowBackground = tga16PaintWindow;
          pScreen->PaintWindowBorder = tga16PaintWindow;
	}
      else
	{
#endif
      ...
      if (!tgaNoAccel)
	{
	  /***** ACCELERATED *****/
	  pScreen->CopyWindow = tgaCopyWindow;
	  pScreen->CreateGC = tgaCreateGC32;
  ...
  pScreen->BlockHandler = tgaBlockHandler;
#endif
