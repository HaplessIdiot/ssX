	      ...
	      LexPtr            val)
{
  ...
  while ((token = xf86GetToken(MuTTab)) != ENDSUBSECTION) {
	...
	if (xf86GetToken(NULL) != STRING)
	  ...
	  for (i = 0; i < max; i++) {
		...
		(strcmp(((MuTPrivatePtr) array[i]->private)->input_dev,
			val->str) == 0)) {
	      DBG(2, ErrorF("MicroTouch config detected a device share between %s and %s\n",
	  ...
	  if (i == max) {
	    ...
	    if (xf86Verbose)
	      ErrorF("%s MicroTouch input port: %s\n",
	...
	xf86ConfigError("MicroTouch device name expected");
      ...
      if (xf86Verbose)
	ErrorF("%s MicroTouch X device name: %s\n",
	...
	xf86ConfigError("MicroTouch screen number expected");
      ...
      if (xf86Verbose)
	ErrorF("%s MicroTouch associated screen: %d\n",
	...
	if (xf86Verbose)
	  ErrorF("%s MicroTouch link speed: %s bps\n",
    ...
    case MAXX:
      ...
      priv->max_x = val->num;
      if (xf86Verbose)
	ErrorF("%s MicroTouch maximum x position: %d\n",
    ...
    case MAXY:
      ...
      priv->max_y = val->num;
      if (xf86Verbose)
	ErrorF("%s Microtouch maximum y position: %d\n",
    ...
    case MINX:
      ...
      priv->min_x = val->num;
      if (xf86Verbose)
	ErrorF("%s MicroTouch minimum x position: %d\n",
    ...
    case MINY:
      ...
      priv->min_y = val->num;
      if (xf86Verbose)
	ErrorF("%s MicroTouch minimum y position: %d\n",
	...
	if (xf86Verbose) {
#if DEBUG
	    ErrorF("%s MicroTouch debug level sets to %d\n", XCONFIG_GIVEN,
		   debug_level);      
#else
	    ErrorF("%s MicroTouch debug not available\n",
	...
	xf86ConfigError("MicroTouch motion history size expected");
      ...
      if (xf86Verbose)
	ErrorF("%s MicroTouch motion history size is %d\n", XCONFIG_GIVEN,
	       local->history_size);      
      break;
	...
	if (xf86Verbose)
	    ErrorF("%s MicroTouch device will always stays core pointer\n",
		   XCONFIG_GIVEN);
	break;
...
#if OBSOLETE
static Bool
xf86MuTCalibrate(MuTPrivatePtr	priv,
		 int		*cur_x,
		 int		*cur_y)
{
  int	width, height;
  ...
  *cur_y = priv->screen_height -
  ...
  if ((*cur_x < (priv->cur_x - 1) || *cur_x > (priv->cur_x + 1)) ||
    ...
    /* Restore previous position for use by PostButtonEvent.
     */
    *cur_x = priv->cur_x;
    *cur_y = priv->cur_y;
    return !Success;
...
xf86MuTReadInput(LocalDevicePtr	local)
{
  ...
  while (num_bytes >= (MuT_REPORT_SIZE-bytes_in_packet)) {
    ...
    if (bytes_in_packet == 5) {
      ...
      /*
       * Emit a motion. If AlwaysCore we need to calibrate
