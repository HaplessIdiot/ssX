% $XConsortium: dbe.tex /main/2 1995/10/09 08:34:28 dpw $
...
\newcommand{\requestname}[1]{{\bf #1}}
\newcommand{\argname}[1]{\mbox{\it #1}}
\newcommand{\typename}[1]{\mbox{\tt #1}}
...
\newenvironment{cfunction}[2]{\begin{sloppypar}\begin{keeptogether}\vspace{5mm}\typename{#1}\\ \cfunctionname{#2}\ (}{)\end{keeptogether}\end{sloppypar}{\hangafter=2 \hangindent=20pt \raggedright\par}}
...
happen instantaneously to the user, with no visual artifacts.  Thus,
...
InputOutput windows, a front buffer, into which an image can be drawn.
...
window.  DBE doesn't provide a request for querying whether a window
...
``obscured'') in the same way.  When an Expose event is generated for
...
Expose event on the window.  When a double-buffered window is exposed,
...
\item It is an error (Window) to pass a \typename{BACKBUFFER} in a
...
ClipByChildren.  If an ancestor of a double-buffered window is drawn
to with a core GC having a subwindow-mode of IncludeInferiors, the
...
pixels that the IncludeInferiors drawing touched.

DBE adds no new events.  DBE doesn't extend the semantics of any
...
contain a \typename{DRAWABLE} (e.g., GraphicsExpose) are generated in
...
window background.  In either case, Expose events are generated for
...
window background in both buffers, and Expose events are generated for
D.
...
don't perform any actions themselves.  They are treated as markers by
...
example, if a swap action of Copied is desired, but only some of the
...
will be the same as if the idiom weren't recognized by the
implementation.
...
\typename{DRAWABLE}s (or a Drawable error results).  For each drawable
...
If the \argname{window} isn't already a double-buffered window, the
...
(or a Window error results).  The window passed into the request must
be an InputOutput window (or a Match error results).  The visual of
the window must be in the list returned by
\requestname{DBEGetVisualInfo} (or a Match error results).  The
\argname{back-buffer-name} must be in the range assigned to the
client, and must not already be in use (or an IDChoice error results).
If the server cannot allocate all resources associated with turning on
double-buffering for the window, an Alloc error results, the window's
double-buffer status (whether it is already double-buffered or not)
...
associated with a window (or a Buffer error results).
...
\typename{SWAPACTION}: {\{Undefined, Background, Untouched, Copied\}}

Each window passed into the request must be a valid \typename{WINDOW}
(or a Window error results).  Each window passed into the request must
be a double-buffered window (or a Match error results).  Each window
passed into the request must only be listed once (or a Match error
results).  Each \argname{swap-action} in the list must have one of the
values specified for type \typename{SWAPACTION} (or a Value error
...
\begin{description}
\item[Undefined]	The contents of the new back buffer become
...
\item[Background]	The unobscured region of the new back buffer
			will be tiled with the window background.  The
			background action allows devices to use a fast
			clear capability during a swap.

\item[Untouched]	The unobscured region of the new back buffer
			will be unmodified by the swap.

\item[Copied]		The unobscured region of the new back buffer
...
the \argname{attributes} in the reply will be None.
...
The type \typename{XdbeBackBuffer} is a \typename{Drawable}.

The type \typename{XdbeSwapAction} can be one of the constants
\constant{XdbeUndefined}, \constant{XdbeBackground},
\constant{XdbeUntouched}, or \constant{XdbeCopied}.
...
All functions that have return type \typename{Status} will return
...
Sets \argname{major\_version\_return} and
...
\typename{XdbeScreenVisualInfo} structures of length \argname{num\_screens}.
...
The \typename{XdbeScreenVisualInfo} structure has the following
fields:
...
The \typename{XdbeVisualInfo} structure has the following fields:
...
This function frees the list of \typename{XdbeScreenVisualInfo} returned by
...
The \typename{XdbeSwapInfo} structure has the following fields:
...
The \typename{XdbeBackBufferAttributes} structure has the following fields:

\begin{tabular}{lll}
\typename{Window} & \argname{window} & window that buffer belongs to \\
\end{tabular}

If \argname{buffer} is not a valid \typename{XdbeBackBuffer}, \argname{window}
is set to \constant{None}.

The returned \typename{XdbeBackBufferAttributes} structure can be
...
The \typename{XdbeBufferError} structure has the following fields:
...
\typename{unsigned char} & \argname{error\_code}& error base + \constant{XdbeBadBuffer}\\
