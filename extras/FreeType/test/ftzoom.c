...
#define  MAXPTSIZE  500                 /* dtp */
  ...
  int  main( int  argc, char**  argv )
  {
    ...
    while ( 1 )
    {
      option = ft_getopt( argc, argv, "e:gp:r:z:" );
      ...
      if ( (error = LoadTrueTypeChar( Num, hinted )) == TT_Err_Ok )
      {
        ...
        if ( use_cmap )
          sprintf( Header, "\"%s\": index = %3d, code = 0x%x, hinting = %s",
                   glyphname, Num, Code, hinted ? "ON" : "OFF" );
        else
          sprintf( Header, "\"%s\": index = %3d, hinting = %s",
                   glyphname, Num, hinted ? "ON" : "OFF" );
      }
      else {
        Fail++;
        sprintf( Header, "\"%s\": index = %3d, hinting = %s (ERROR %lx)",
                 glyphname, Num, hinted ? "ON" : "OFF", error );
      }
