...
#define RENDER_Gray_Glyph( glyph, target, palette ) \
  ...
  const TT_Instance   TT_Null_Instance = { (void*)0 };
 ...
 ******************************************************************/
  ...
  TT_Error  TT_Init_FreeType( TT_Engine*  engine )
  {
    ...
    /* first of all, allocate engine instance */
    if ( ALLOC( _engine, sizeof ( TEngine_Instance ) ) )
      return error;

    /* Initalize components */
    if ( ( error = TTMemory_Init( _engine ) )  ||
         ( error = TTFile_Init  ( _engine ) )  ||
         ( error = TTLists_Init ( _engine ) )  ||
#ifdef TT_EXTEND_ENGINE
         ( error = TTExtend_Init( _engine ) )  ||
#endif
         ( error = TTObjs_Init  ( _engine ) )  ||
         ( error = TTRaster_Init( _engine ) )  )
      goto Fail;

    /* set the gray palette defaults: 0 to 4 */
    for ( n = 0; n < 5; n++ )
       _engine->raster_palette[n] = n;
...
#endif
    TTLists_Done ( _engine );
    TTFile_Done  ( _engine );
    TTMemory_Done( _engine );
 ...
 *                 smoothing.
 *
 *  Input  :  palette       address of palette (a 5 byte array)
 *
 ...
 ******************************************************************/
  ...
  {  
    ...
    for ( i = 0; i < 5; i++ )
      HANDLE_Engine( engine )->raster_palette[i] = palette[i];
 ...
 *
 *  Input  :  instance      the instance handle
 *            pointsize     new point size in 26.6 format
 *
 *  Output :  Error code.
 *
 *  NOTE : y_resolution is currently ignored.
 ...
 ******************************************************************/
                                     ...
                                     TT_Instance_Metrics*  metrics )
  {
    ...
    metrics->x_scale      = MulDiv_Round( 0x10000,
                                          ins->metrics.x_scale1,
                                          ins->metrics.x_scale2 );

    metrics->y_scale      = MulDiv_Round( 0x10000,
 ...
 ******************************************************************/
                          ...
                          TT_Glyph*  glyph )
  {
    ...
    /* get a new glyph from the face's cache */
 ...
 *
 *  Description :  Extracts the glyph's metrics information.
 *
 *  Input  :  None
 *
 *  Output :  Error code.
 ...
 *
 *  Description :  Creates a new TrueType outline, reservinf
 *                 array space for a given number of points and
 *                 contours
 *
 *  Input  :  num_points     number of points
 ...
 ******************************************************************/
                            ...
                            TT_Outline*  outline )
  {
    ...
    if ( num_points < 0 || num_contours < 0 || !outline )
      return TT_Err_Invalid_Argument;

    *outline = null_outline;

    l = num_points*sizeof(TT_F26Dot6);

    if ( ALLOC( outline->xCoord,  num_points*sizeof(TT_F26Dot6) ) ||
         ALLOC( outline->yCoord,  num_points*sizeof(TT_F26Dot6) ) ||
         ALLOC( outline->flag,    num_points*sizeof(Byte)       ) ||
         ALLOC( outline->conEnds, num_contours*sizeof(UShort) ) )
      goto Fail;

    outline->points   = num_points;
    outline->contours = num_contours;
    outline->owner    = TRUE;
    return TT_Err_Ok;
 ...
 *                 calling this function for the outlines returned
 *                 by TT_Get_Glyph_Outline is an error
 *
 ...
 ******************************************************************/
                                   ...
                                   TT_Raster_Map*  raster_map )
  {
    ...
    if ( !outline || !raster_map )
      return TT_Err_Invalid_Argument;

    return RENDER_Glyph( outline, raster_map );
  }
 ...
 ******************************************************************/
                                   ...
                                   TT_Raster_Map*  raster_map )
  {
    ...
    return RENDER_Gray_Glyph( outline, raster_map, _engine->raster_palette );
  }
 ...
 ******************************************************************/
                             ...
                             TT_Outline*  target )
  {
    if ( !source || !target || 
         ...
         source->contours != target->contours )
      return TT_Err_Invalid_Argument;

    MEM_Copy( target->xCoord, source->xCoord, 
              source->points*sizeof(TT_F26Dot6) );

    MEM_Copy( target->yCoord, source->yCoord, 
              source->points*sizeof(TT_F26Dot6) );
    
    MEM_Copy( target->flag, source->flag, source->points*sizeof(Byte));

    MEM_Copy( target->conEnds, source->conEnds,
              source->contours*sizeof(UShort) );
 ...
 ******************************************************************/
                             ...
                             TT_Matrix*   matrix )
  {
    ...
    xz = MulDiv_Round( *x, matrix->xx, 0x10000 ) +
         MulDiv_Round( *y, matrix->xy, 0x10000 );

    yz = MulDiv_Round( *x, matrix->yx, 0x10000 ) +
         MulDiv_Round( *y, matrix->yy, 0x10000 );
 ...
 ******************************************************************/
                                 ...
                                 TT_BBox*     bbox )
  {
    ...
    if (outline && bbox)
    {
      ...
      {
        bbox->xMin = bbox->xMax = outline->xCoord[0];
        bbox->yMin = bbox->yMax = outline->yCoord[0];

        for ( k = 1; k < outline->points; k++ )
        {
          x = outline->xCoord[k];
 ...
 *  Output :  glyph index.
 *
 *  Notes  :  Char code 0 is the unknown glyph, which should never
 *            be displayed.
 *
 ******************************************************************/
