...
/****************************************************************************/
  ...
  static Bool  Decompose_Curve( RAS_ARGS Int  first, 
                                         Int  last )
  {    
    ...
    x_first = SCALED( ras.x_coord[first] );
    y_first = SCALED( ras.y_coord[first] );

    x_last  = SCALED( ras.x_coord[last] );
    y_last  = SCALED( ras.y_coord[last] );
    ...
    {
      index++;
      x = SCALED( ras.x_coord[index] );
      y = SCALED( ras.y_coord[index] );
...
/****************************************************************************/
  ...
  static Bool  Convert_Glyph( RAS_ARGS PStorage  _xCoord, PStorage  _yCoord )
  {
    ...
    for ( i = 0; i < ras.nContours; i++ )
    {
      ...
      if ( Decompose_Curve( RAS_VARS  start, ras.outs[i]) ) return FAILURE;
...
/***********************************************************************/
  ...
  static void  Vertical_Gray_Sweep_Init( RAS_ARGS int*  min, int*  max )
  {
    *min = *min & -2;
    *max = ( *max+3 ) & -2;
  ...
  static void  Horizontal_Gray_Sweep_Span( RAS_ARGS int         y,
                                                    TT_F26Dot6  x1,
                                                    TT_F26Dot6  x2,
                                                    PProfile    left,
                                                    PProfile    right )
  ...
  static void  Horizontal_Gray_Sweep_Drop( RAS_ARGS int         y,
                                                    TT_F26Dot6  x1,
                                                    TT_F26Dot6  x2,
                                                    PProfile    left,
                                                    PProfile    right )
...
/****************************************************************************/
                                   ...
                                   TT_Raster_Map*  target_map )
  {
	...
	if ( glyph->points <= 0 || glyph->contours <= 0 )
	  return TT_Err_Ok;

    if ( glyph->points < glyph->conEnds[glyph->contours-1] )
    {
...
/****************************************************************************/
                                         ...
                                         char*           palette )
  {
    ...
    error = Render_Single_Pass( RAS_VARS  glyph->xCoord, glyph->yCoord );
    if (error)
      return error;
    ...
    if ( ras.second_pass && ras.dropOutControl != 0 )
    {
      ...
      error = Render_Single_Pass( RAS_VARS glyph->yCoord, glyph->xCoord );
      if (error)
        return error;
    }
