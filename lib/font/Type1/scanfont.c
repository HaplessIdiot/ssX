...
static EncodingTable ISO8859_2Enc[] = {
  32, "space",
  33, "exclam",
  34, "quotedbl",
  35, "numbersign",
...
boolean Init_BuiltInEncoding()
{
    ...
    ISOLatin2EncArrayP = MakeEncodingArrayP(ISO8859_2Enc);
    return (StdEncArrayP && ISOLatin1EncArrayP && ISOLatin2EncArrayP);
}
    ...
    psobj *arrayP;
{
  ...
  if ((tokenType == TOKEN_NAME && (tokenLength==16 || tokenLength==17)))
  {
    ...
    else if((tokenLength==17) && (!strncmp(tokenStartP,"ISOLatin2Encoding",17)))
	  arrayP->data.valueP = (char *) ISOLatin2EncArrayP;
    else
